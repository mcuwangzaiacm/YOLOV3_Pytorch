...
def main():
    ...
    #                   如果不设置model_path，pretrained = True，此时仅加载主干 darknet53_backbone_weights.pth 开始训练。
    ...
    #   获得图片路径和标签
    # ----------------------------------------------------#
    train_annotation_path = 'CapData/train.txt'
    val_annotation_path = 'CapData/val.txt'
    ...
    if local_rank == 0:
        ...
        loss_history    = None
    #------------------------------------------------------------------#
    #   torch 1.2不支持amp，建议使用torch 1.7.1及以上正确使用fp16(混合精度训练)
    ...
    if local_rank == 0:
        ...
        if total_step <= wanted_step:
            ...
            print("[Warning] 使用%s优化器时，建议将训练总步长设置到%d以上。"%(optimizer_type, wanted_step))
            print("[Warning] 本次运行的总训练数据量为%d，Unfreeze_batch_size为%d，共训练%d个Epoch，计算出总训练步长为%d。"%(num_train, Unfreeze_batch_size, UnFreeze_Epoch, total_step))
            print("[Warning] 由于总训练步长为%d，小于建议总步长%d，建议设置总世代为%d。"%(total_step, wanted_step, wanted_epoch))
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            #     以上述参数为例  epoch 在[0, 50] 进行冻结训练,  [51, 300] 进行解冻训练
            # ---------------------------------------#
